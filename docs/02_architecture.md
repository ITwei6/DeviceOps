...
- 接入层：负责设备连接、MQTT 消息接入、协议解析和数据标准化。
...
| 项目 | 说明 |
| --- | --- |
| 职责 | 负责设备接入、MQTT 消息订阅、设备消息解析、基础校验和标准化转发。 |
| 输入 | 设备 MQTT 消息、设备心跳、设备状态数据、错误码、设备运行日志。 |
| 输出 | 标准化设备状态消息、设备日志消息、设备在线离线事件、协议解析错误日志。 |
| 依赖 | MQTT Broker、device-service、telemetry-service、log-service、RabbitMQ、etcd。 |
| 技术 | C++17、MQTT、brpc、protobuf、RabbitMQ Client、cpp-microservice-kit 基础能力。 |

设计说明：

- device-gateway 是设备侧流量进入平台的第一站，不承担复杂业务判断。
- 设备消息进入后先做协议解析、字段校验和设备身份识别。
- 标准化后的状态数据同步发送给 telemetry-service，重要事件可异步投递 RabbitMQ。
